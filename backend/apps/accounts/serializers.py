...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = (
            ...
            "role", "status", "phone", "is_phone_verified", "bio", "avatar",
            "locale", "theme", "date_joined",
        )
        read_only_fields = ("id", "status", "is_phone_verified", "date_joined", "role")
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_role(self, value):
        if value in (User.Role.ADMIN, User.Role.SUPER_ADMIN):
            raise serializers.ValidationError("Bu rol ro'yxatdan o'tishda tanlanmaydi.")
        return value
